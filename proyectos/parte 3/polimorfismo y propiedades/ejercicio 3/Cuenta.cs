...
namespace ejercicio3
{
    class Cuenta
    {
        protected NumeroCuenta numero {get; private set;}
        ...
        virtual public void Ingreso(double cantidad)
        {
            Saldo += cantidad;
            Console.Write($"Ingreso de {cantidad:F2} euros: ");
        }

        virtual public double Reintegro(double cantidad)
        {
            if (cantidad > Saldo || cantidad == 0)
            {
                throw new SaldoInsuficienteException($"No puede hacer un reintegro de {cantidad:F2} euros/s porque la suma es superior al saldo disponible.");
            }
            else
            {
                Console.Write($"Reintegro de {cantidad:F2} euros: ");
                Saldo -= cantidad;
            }
            return cantidad;
        }
        ...
        {
            string mensaje = $"Número de cuenta: {numero}" +
                             $"\nTitular: {titular}" + 
                             $"\nSaldo: {Saldo:F2} euros";                   
            return mensaje;
        }
    }
}
