    ...
    class CuentaCredito : Cuenta
    {
        ...
        public class CreditoMaximoExcedidoException : Exception
        {
            public CreditoMaximoExcedidoException(double creditoMaximo_Euros, double creditoConcedidoHastaAhora_Euros, double cantidadAReintegrar_Euros) :
                base($"No puedes retirar {cantidadAReintegrar_Euros:F2}." +
                     $"\nTu crédito máximo es de {creditoMaximo_Euros:F2} del cual se te ha concedido ya {creditoConcedidoHastaAhora_Euros}")
            {
            }
        }
        ...
        {
            if (Saldo - cantidad < 0 && Math.Abs(Saldo - cantidad) > creditoMaximo_Euros)
            {
                throw new CreditoMaximoExcedidoException(creditoMaximo_Euros, Math.Abs(Saldo), cantidad);
            }     
            Saldo -= cantidad;
            return cantidad;
        }
