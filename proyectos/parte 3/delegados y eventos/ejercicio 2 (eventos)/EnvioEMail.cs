...
namespace ejercicio2Eventos
{
    class EnvioEMail
    {
        public void EnviarAvisoRetiradaDeEfectivo(object sender, RetiradaDeEfectivoEventArgs e)
        {
            Console.WriteLine($"Buscando datos del usuario con DNI {e.Dni}...");
            Console.WriteLine($"Enviando email al usuario con DNI {e.Dni}" +
                              $"\ninformándole de la retirada de {e.Cantidad} euros" + 
                              $"\nel día {DateTime.Now.ToString("dd/MM/yyyy")} " +
                              $"a las {DateTime.Now.ToString("HH:mm")} horas.\n");
        }
    }
}